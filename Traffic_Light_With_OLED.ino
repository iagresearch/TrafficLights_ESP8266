...
//
//The OLED display uses ADAFRUIT libraries - GFX and SSD1306 - Add the Adafruit libraries under Sketch -> Include Libraries ->Manage Libraries
//I used a HiLetGo 0.96" I2C 128x64 OLED display from Amazon for POC / Development ($5.86 ea)
...
const int pinLightOrange  = D8;   
const int pinLightGreen   = D7;   //Must avoid SCL & SDA lines, as used by I2C for screen!
const int pinLightBus     = D10;
...
void setup(){
 ...
 // Show image buffer on the display hardware.
  ...
  display.println("Traffic Lights");
  display.display();
  display.setCursor(0,9);
  display.println("Setting GPIO Pins");
  display.display();
  ...
  display.println("Checking Boot Mode");
  display.display();
  ...
  if(LocalOps == 0) {
    ...
    display.setCursor(0,0);
    display.println("AUTO Traffic Lights");
    display.setCursor(0,9);
    display.println("Sequence:");
    display.setCursor(0,18);
    display.println("15s-1.5s-15s");
    display.display();
  ...
  display.clearDisplay();
  display.setCursor(0,0);
  display.println("Web Traffic Lights");
  display.setCursor(0,9);
